//
//  NSLayoutConstraint+HWMASDebugAdditions.m
//  HWMASonry
//
...
#import "NSLayoutConstraint+HWMASDebugAdditions.h"
#import "HWMASConstraint.h"
#import "HWMASLayoutConstraint.h"

@implementation NSLayoutConstraint (HWMASDebugAdditions)
...
+ (NSDictionary *)layoutPriorityDescriptionsByValue {
    ...
    dispatch_once(&once, ^{
        ...
        descriptionMap = @{
            @(HWMASLayoutPriorityDefaultHigh)      : @"high",
            @(HWMASLayoutPriorityDefaultLow)       : @"low",
            @(HWMASLayoutPriorityDefaultMedium)    : @"medium",
            @(HWMASLayoutPriorityRequired)         : @"required",
            @(HWMASLayoutPriorityFittingSizeLevel) : @"fitting size",
        };
#elif TARGET_OS_MAC
        descriptionMap = @{
            @(HWMASLayoutPriorityDefaultHigh)                 : @"high",
            @(HWMASLayoutPriorityDragThatCanResizeWindow)     : @"drag can resize window",
            @(HWMASLayoutPriorityDefaultMedium)               : @"medium",
            @(HWMASLayoutPriorityWindowSizeStayPut)           : @"window size stay put",
            @(HWMASLayoutPriorityDragThatCannotResizeWindow)  : @"drag cannot resize window",
            @(HWMASLayoutPriorityDefaultLow)                  : @"low",
            @(HWMASLayoutPriorityFittingSizeCompression)      : @"fitting size",
            @(HWMASLayoutPriorityRequired)                    : @"required",
        };
#endif
    });
...
+ (NSString *)descriptionForObject:(id)obj {
    if ([obj respondsToSelector:@selector(HWMAS_key)] && [obj HWMAS_key]) {
        return [NSString stringWithFormat:@"%@:%@", [obj class], [obj HWMAS_key]];
    }
...
- (NSString *)description {
    ...
    if (self.priority != HWMASLayoutPriorityRequired) {
