...
#error HWSVProgressHUD is ARC only. Either turn on ARC for the project or use -fobjc-arc flag
#endif

#import "HWSVProgressHUD.h"
#import "HWSVIndefiniteAnimatedView.h"
#import "HWSVProgressAnimatedView.h"
#import "HWSVRadialGradientLayer.h"

NSString * const HWSVProgressHUDDidReceiveTouchEventNotification = @"HWSVProgressHUDDidReceiveTouchEventNotification";
NSString * const HWSVProgressHUDDidTouchDownInsideNotification = @"HWSVProgressHUDDidTouchDownInsideNotification";
NSString * const HWSVProgressHUDWillDisappearNotification = @"HWSVProgressHUDWillDisappearNotification";
NSString * const HWSVProgressHUDDidDisappearNotification = @"HWSVProgressHUDDidDisappearNotification";
NSString * const HWSVProgressHUDWillAppearNotification = @"HWSVProgressHUDWillAppearNotification";
NSString * const HWSVProgressHUDDidAppearNotification = @"HWSVProgressHUDDidAppearNotification";

NSString * const HWSVProgressHUDStatusUserInfoKey = @"HWSVProgressHUDStatusUserInfoKey";

static const CGFloat HWSVProgressHUDParallaxDepthPoints = 10.0f;
static const CGFloat HWSVProgressHUDUndefinedProgress = -1;
static const CGFloat HWSVProgressHUDDefaultAnimationDuration = 0.15f;
static const CGFloat HWSVProgressHUDVerticalSpacing = 12.0f;
static const CGFloat HWSVProgressHUDHorizontalSpacing = 12.0f;
static const CGFloat HWSVProgressHUDLabelSpacing = 8.0f;


@interface HWSVProgressHUD ()
...
@property (nonatomic, strong) HWSVRadialGradientLayer *backgroundRadialGradientLayer;
...
@property (nonatomic, strong) HWSVProgressAnimatedView *ringView;
@property (nonatomic, strong) HWSVProgressAnimatedView *backgroundRingView;
...
@implementation HWSVProgressHUD {
    BOOL _isInitializing;
}

+ (HWSVProgressHUD*)sharedView {
    static dispatch_once_t once;
    
    static HWSVProgressHUD *sharedView;
#if !defined(HWSV_APP_EXTENSIONS)
...
+ (void)setDefaultStyle:(HWSVProgressHUDStyle)style {
    [self sharedView].defaultStyle = style;
}

+ (void)setDefaultMaskType:(HWSVProgressHUDMaskType)maskType {
    [self sharedView].defaultMaskType = maskType;
}

+ (void)setDefaultAnimationType:(HWSVProgressHUDAnimationType)type {
...
+ (void)setForegroundColor:(UIColor*)color {
    ...
    [self setDefaultStyle:HWSVProgressHUDStyleCustom];
}
...
+ (void)showWithMaskType:(HWSVProgressHUDMaskType)maskType {
    HWSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
...
+ (void)showWithStatus:(NSString*)status {
    [self showProgress:HWSVProgressHUDUndefinedProgress status:status];
}

+ (void)showWithStatus:(NSString*)status maskType:(HWSVProgressHUDMaskType)maskType {
    HWSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress maskType:(HWSVProgressHUDMaskType)maskType {
    HWSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showProgress:(float)progress status:(NSString*)status maskType:(HWSVProgressHUDMaskType)maskType {
    HWSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showInfoWithStatus:(NSString*)status maskType:(HWSVProgressHUDMaskType)maskType {
    HWSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showSuccessWithStatus:(NSString*)status maskType:(HWSVProgressHUDMaskType)maskType {
    HWSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showErrorWithStatus:(NSString*)status maskType:(HWSVProgressHUDMaskType)maskType {
    HWSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)showImage:(UIImage*)image status:(NSString*)status maskType:(HWSVProgressHUDMaskType)maskType {
    HWSVProgressHUDMaskType existingMaskType = [self sharedView].defaultMaskType;
    [self setDefaultMaskType:maskType];
...
+ (void)dismissWithCompletion:(HWSVProgressHUDDismissCompletion)completion {
...
+ (void)dismissWithDelay:(NSTimeInterval)delay completion:(HWSVProgressHUDDismissCompletion)completion {
...
- (instancetype)initWithFrame:(CGRect)frame {
    if((self = [super initWithFrame:frame])) {
        ...
        // Set default values
        _defaultMaskType = HWSVProgressHUDMaskTypeNone;
        _defaultStyle = HWSVProgressHUDStyleLight;
        _defaultAnimationType = HWSVProgressHUDAnimationTypeFlat;
        _minimumSize = CGSizeZero;
        _font = [UIFont preferredFontForTextStyle:UIFontTextStyleSubheadline];
        
//        NSBundle *bundle = [NSBundle bundleForClass:[HWSVProgressHUD class]];
//        NSURL *url = [bundle URLForResource:@"HWSVProgressHUD" withExtension:@"bundle"];
//        NSBundle *imageBundle = [NSBundle bundleWithURL:url];
        NSBundle *imageBundle ;
        NSString *path = [[NSBundle mainBundle]pathForResource:@"HWSVProgressHUD" ofType:@"bundle"];
        ...
        _fadeInAnimationDuration = HWSVProgressHUDDefaultAnimationDuration;
        _fadeOutAnimationDuration = HWSVProgressHUDDefaultAnimationDuration;
        ...
        self.accessibilityIdentifier = @"HWSVProgressHUD";
...
- (void)updateHUDFrame {
    ...
    hudWidth = HWSVProgressHUDHorizontalSpacing + MAX(labelWidth, contentWidth) + HWSVProgressHUDHorizontalSpacing;
    
    // |-spacing-content-(labelSpacing-label-)spacing-|
    hudHeight = HWSVProgressHUDVerticalSpacing + labelHeight + contentHeight + HWSVProgressHUDVerticalSpacing;
    if(self.statusLabel.text && (imageUsed || progressUsed)){
        // Add spacing if both content and label are used
        hudHeight += HWSVProgressHUDLabelSpacing;
    }
    ...
    CGFloat centerY;
    if(self.statusLabel.text) {
        CGFloat yOffset = MAX(HWSVProgressHUDVerticalSpacing, (self.minimumSize.height - contentHeight - HWSVProgressHUDLabelSpacing - labelHeight) / 2.0f);
    ...
    if(self.progress != HWSVProgressHUDUndefinedProgress) {
    ...
    // Label
    if(imageUsed || progressUsed) {
        centerY = CGRectGetMaxY(imageUsed ? self.imageView.frame : self.indefiniteAnimatedView.frame) + HWSVProgressHUDLabelSpacing + labelHeight / 2.0f;
    } else {
        centerY = CGRectGetMidY(self.hudView.bounds);
    }
...
- (void)updateMotionEffectForXMotionEffectType:(UIInterpolatingMotionEffectType)xMotionEffectType yMotionEffectType:(UIInterpolatingMotionEffectType)yMotionEffectType {
    ...
    effectX.minimumRelativeValue = @(-HWSVProgressHUDParallaxDepthPoints);
    effectX.maximumRelativeValue = @(HWSVProgressHUDParallaxDepthPoints);
    
    UIInterpolatingMotionEffect *effectY = [[UIInterpolatingMotionEffect alloc] initWithKeyPath:@"center.y" type:yMotionEffectType];
    effectY.minimumRelativeValue = @(-HWSVProgressHUDParallaxDepthPoints);
    effectY.maximumRelativeValue = @(HWSVProgressHUDParallaxDepthPoints);
...
#else
            // If HWSVProgressHUD is used inside an app extension add it to the given view
...
- (NSDictionary*)notificationUserInfo {
    return (self.statusLabel.text ? @{HWSVProgressHUDStatusUserInfoKey : self.statusLabel.text} : nil);
}
...
#if !defined(HWSV_APP_EXTENSIONS) && TARGET_OS_IOS
    self.frame = [[[UIApplication sharedApplication] delegate] window].bounds;
    UIInterfaceOrientation orientation = UIApplication.sharedApplication.statusBarOrientation;
#elif !defined(HWSV_APP_EXTENSIONS) && !TARGET_OS_IOS
...
- (void)controlViewDidReceiveTouchEvent:(id)sender forEvent:(UIEvent*)event {
    [[NSNotificationCenter defaultCenter] postNotificationName:HWSVProgressHUDDidReceiveTouchEventNotification
                                                        object:self
    ...
    if(CGRectContainsPoint(self.hudView.frame, touchLocation)) {
        [[NSNotificationCenter defaultCenter] postNotificationName:HWSVProgressHUDDidTouchDownInsideNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
    }
}
...
- (void)showProgress:(float)progress status:(NSString*)status {
    __weak HWSVProgressHUD *weakSelf = self;
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        __strong HWSVProgressHUD *strongSelf = weakSelf;
        if(strongSelf){
            // Stop timer
...
- (void)showImage:(UIImage*)image status:(NSString*)status duration:(NSTimeInterval)duration {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            strongSelf.progress = HWSVProgressHUDUndefinedProgress;
            [strongSelf cancelRingLayerAnimation];
            [strongSelf cancelIndefiniteAnimatedViewAnimation];
...
- (void)fadeIn:(id)data {
    ...
    if(self.defaultMaskType != HWSVProgressHUDMaskTypeNone) {
    ...
    if(self.backgroundView.alpha != 1.0f) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:HWSVProgressHUDWillAppearNotification
                                                            object:self
                                                          userInfo:[self notificationUserInfo]];
        ...
        __block void (^completionBlock)(void) = ^{
            ...
            if(self.backgroundView.alpha == 1.0f){
                ...
                // Post notification to inform user
                [[NSNotificationCenter defaultCenter] postNotificationName:HWSVProgressHUDDidAppearNotification
                                                                    object:self
                                                                  userInfo:[self notificationUserInfo]];
...
- (void)dismissWithDelay:(NSTimeInterval)delay completion:(HWSVProgressHUDDismissCompletion)completion {
    ...
    [[NSOperationQueue mainQueue] addOperationWithBlock:^{
        ...
        if(strongSelf){
            ...
            // Post notification to inform user
            [[NSNotificationCenter defaultCenter] postNotificationName:HWSVProgressHUDWillDisappearNotification
                                                                object:nil
            ...
            __block void (^completionBlock)(void) = ^{
                ...
                if(self.backgroundView.alpha == 0.0f){
                    ...
                    // Post notification to inform user
                    [[NSNotificationCenter defaultCenter] postNotificationName:HWSVProgressHUDDidDisappearNotification
                                                                        object:strongSelf
                                                                      userInfo:[strongSelf notificationUserInfo]];
...
- (UIView*)indefiniteAnimatedView {
    ...
    if(self.defaultAnimationType == HWSVProgressHUDAnimationTypeFlat){
        // Check if spinner exists and is an object of different class
        if(_indefiniteAnimatedView && ![_indefiniteAnimatedView isKindOfClass:[HWSVIndefiniteAnimatedView class]]){
            [_indefiniteAnimatedView removeFromSuperview];
            _indefiniteAnimatedView = nil;
        }
        
        if(!_indefiniteAnimatedView){
            _indefiniteAnimatedView = [[HWSVIndefiniteAnimatedView alloc] initWithFrame:CGRectZero];
        }
        
        // Update styling
        HWSVIndefiniteAnimatedView *indefiniteAnimatedView = (HWSVIndefiniteAnimatedView*)_indefiniteAnimatedView;
...
- (HWSVProgressAnimatedView*)ringView {
    if(!_ringView) {
        _ringView = [[HWSVProgressAnimatedView alloc] initWithFrame:CGRectZero];
    }
...
- (HWSVProgressAnimatedView*)backgroundRingView {
    if(!_backgroundRingView) {
        _backgroundRingView = [[HWSVProgressAnimatedView alloc] initWithFrame:CGRectZero];
...
- (UIColor*)foregroundColorForStyle {
    if(self.defaultStyle == HWSVProgressHUDStyleLight) {
        return [UIColor blackColor];
    } else if(self.defaultStyle == HWSVProgressHUDStyleDark) {
...
-(UIView *)backgroundView {
    ...
    // Update styling
    if(self.defaultMaskType == HWSVProgressHUDMaskTypeGradient){
        if(!_backgroundRadialGradientLayer){
            _backgroundRadialGradientLayer = [HWSVRadialGradientLayer layer];
        }
        ...
        if(self.defaultMaskType == HWSVProgressHUDMaskTypeBlack){
            _backgroundView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4];
        } else if(self.defaultMaskType == HWSVProgressHUDMaskTypeCustom){
...
- (UIWindow *)frontWindow {
    ...
    for (UIWindow *window in frontToBackWindows) {
        ...
        BOOL windowIHWSVisible = !window.hidden && window.alpha > 0;
        BOOL windowLevelSupported = (window.windowLevel >= UIWindowLevelNormal && window.windowLevel <= self.maxSupportedWindowLevel);
        BOOL windowKeyWindow = window.isKeyWindow;
			
        if(windowOnMainScreen && windowIHWSVisible && windowLevelSupported && windowKeyWindow) {
            return window;
        }
    }
#endif
...
- (void)fadeInEffects {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000
    if(self.defaultStyle != HWSVProgressHUDStyleCustom) {
        // Add blur effect
        UIBlurEffectStyle blurEffectStyle = self.defaultStyle == HWSVProgressHUDStyleDark ? UIBlurEffectStyleDark : UIBlurEffectStyleLight;
...
- (void)setDefaultAnimationType:(HWSVProgressHUDAnimationType)animationType {
